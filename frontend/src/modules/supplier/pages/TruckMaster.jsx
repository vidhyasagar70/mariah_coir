  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card-panel p-6 rounded-2xl space-y-4">
        <h4 className="text-xs font-bold text-[#2E1C11] uppercase tracking-wider">Add New Master Vehicle Type</h4>
        ...
        <form onSubmit={handleAddMasterVehicle} className="flex flex-col sm:flex-row items-stretch sm:items-end gap-3 text-xs">
          <div className="w-full sm:w-1/2">
            <label className="block text-[#3D281C] font-semibold mb-1">Vehicle Type / Description *</label>
            <input
              type="text"
              required
              ...
              className="w-full px-3 py-2 rounded-xl bg-[#FAF7F2] border border-[#E8DCD0] text-[#2E1C11] placeholder-[#A8988B] focus:outline-none focus:border-[#8C5E3C]"
            />
          </div>

          <div className="w-full sm:w-1/3">
            <label className="block text-[#3D281C] font-semibold mb-1">Default Rate Per Trip (₹)</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 rounded-xl bg-[#FAF7F2] border border-[#E8DCD0] text-[#2E1C11] font-mono font-bold placeholder-[#A8988B] focus:outline-none focus:border-[#8C5E3C]"
              />
            </div>
          </div>
          ...
          <div className="w-full sm:w-auto pt-1 sm:pt-0">
            <button
              type="submit"
              disabled={submitting}
              className="w-full sm:w-auto flex items-center justify-center space-x-2 px-5 py-2 rounded-xl bg-[#965E36] hover:bg-[#7A4A28] text-white font-bold cursor-pointer disabled:opacity-50 transition-all shadow-xs"
            >
      ...
      <div className="card-panel rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center space-y-3">
            ...
            <h3 className="text-sm font-bold text-[#2E1C11]">No Vehicle Types Configured</h3>
            <p className="text-xs text-[#7A6759]">Add standard vehicle types above to configure trip rate matrices.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-[#F4EDE4]">
                {masterVehicles.map((mv) => (
                  <tr key={mv.id} className="hover:bg-[#FAF7F2]/80 transition-colors">
                    <td className="p-4 font-bold text-[#2E1C11] flex items-center space-x-2">
                      <Truck className="h-4 w-4 text-[#7A6759]" />
                      <span>{mv.vehicle_type}</span>
                    </td>
                    <td className="p-4 font-mono font-bold text-[#2E1C11] text-sm">
                      {formatCurrency(mv.default_rate)} <span className="text-[11px] font-normal text-[#A8988B]">/ trip</span>
                    </td>
                    <td className="p-4 text-[#7A6759]">
