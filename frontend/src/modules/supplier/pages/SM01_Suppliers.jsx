...
import { getCategoryBadgeClass, getStatusBadgeClass } from '../../../shared/utils/formatters';
      ...
      <div className="card-panel p-4 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-3">
        ...
        <div className="grid grid-cols-2 gap-3 w-full md:w-auto">
          ...
          <select
            ...
            className="px-3 py-1.5 rounded-xl bg-[#FAF7F2] border border-[#E8DCD0] text-xs text-[#2E1C11] focus:outline-none focus:border-[#8C5E3C] font-medium"
          >
      ...
      <div className="card-panel rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center space-y-3">
            ...
            <h3 className="text-sm font-bold text-[#2E1C11]">No Suppliers Found</h3>
            <p className="text-xs text-[#7A6759]">Add a new supplier or adjust search filters.</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-[#F4EDE4]">
                {suppliers.map((s) => (
                  <tr key={s.id} className="hover:bg-[#FAF7F2]/80 transition-colors">
                    {/* SUPPLIER ID */}
                    <td className="p-4 font-mono font-bold text-[#2E1C11] whitespace-nowrap">
                      {s.id}
                    </td>

                    {/* NAME / COMPANY */}
                    <td className="p-4">
                      <div className="font-bold text-[#2E1C11] text-sm">{s.name}</div>
                      {s.company_name && (
                        <div className="text-[11px] text-[#7A6759] flex items-center space-x-1 mt-0.5">
                          <Building2 className="h-3 w-3 text-[#A8988B]" />
                    ...
                    {/* CONTACT */}
                    <td className="p-4">
                      <div className="font-semibold text-[#2E1C11] flex items-center space-x-1">
                        <Phone className="h-3.5 w-3.5 text-[#A8988B]" />
                        <span>{s.contact_number}</span>
                      </div>
                      {s.contact_person && (
                        <div className="text-[11px] text-[#7A6759] font-normal">Contact: {s.contact_person}</div>
                      )}
                    </td>
                      ...
                      <div className="flex flex-wrap gap-1 max-w-xs">
                        {s.vehicles && s.vehicles.length > 0 ? (
                          s.vehicles.map((v) => (
                            <span
                              key={v.id}
                              className="px-2 py-0.5 rounded-md bg-[#FAF0E6] border border-[#E8D6C5] text-[11px] font-medium text-[#8C532E] whitespace-nowrap"
                            >
                              {v.vehicle_type}: <span className="font-mono font-bold text-[#2E1C11]">₹{v.rate_per_trip}</span>
                            </span>
                          ))
                        ) : (
                          <span className="text-[#A8988B] text-[11px]">No vehicles set</span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end space-x-1">
                        <button
                          ...
                          className="p-1.5 rounded-lg text-[#7A6759] hover:text-[#965E36] hover:bg-[#FAF0E6] transition-colors cursor-pointer"
                          title="Edit Supplier"
                        >
