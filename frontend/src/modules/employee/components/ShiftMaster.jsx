...
export default function ShiftMaster() {
  ...
  const fetchShifts = async () => {
    try {
      ...
      if (res.ok) {
        ...
        showNotification(data.error || 'Failed to fetch shifts', 'error');
      }
    } catch (err) {
      showNotification('Network error while fetching shifts', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveShift = async (formData) => {
    ...
    showNotification(data.message || 'Shift saved successfully.');
    fetchShifts();
  };
  ...
  const handleDelete = async (shift) => {
    ...
    try {
      ...
      if (res.ok) {
        showNotification(data.message || 'Shift deleted successfully.');
        fetchShifts();
      } else {
        showNotification(data.error || 'Failed to delete shift.', 'error');
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-[#FAF7F2] rounded-2xl border border-[#D6C4B0] shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-8 text-center text-xs text-[#7C5A3E] font-medium">Loading shift master data...</div>
        ) : shifts.length === 0 ? (
          <div className="p-12 text-center space-y-2">
            <Clock className="h-8 w-8 text-[#A88B73] mx-auto" />
            <p className="text-sm font-bold text-[#3D2514]">No Shifts Found</p>
            <p className="text-xs text-[#7C5A3E]">Click "+ Add Shift" to configure working shift hours.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-[#EAE0D5]">
                {shifts.map((s) => {
                  const isActive = s.status == 1 || s.status === true;
                  return (
                    <tr key={s.id} className="hover:bg-[#F5EBE0] transition-colors">
                      <td className="py-3 px-4 font-bold text-[#2E1A0C]">{s.name}</td>
                      <td className="py-3 px-4 text-[#3D2514] font-mono font-medium">
                        {s.start_time} - {s.end_time}
                      </td>
                      <td className="py-3 px-4 text-[#6B4B32]">{s.break_duration ? `${s.break_duration} mins` : 'None'}</td>
                      <td className="py-3 px-4 text-[#6B4B32]">{s.description || '—'}</td>
