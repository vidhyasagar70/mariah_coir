...
export default function PositionModal({ isOpen, onClose, onSave, position }) {
  ...
  useEffect(() => {
    if (position) {
      ...
      setStatus(position.status == 1 || position.status === true);
    } else {
      setName('');
      setDescription('');
      setStatus(true);
        ...
        <div className="bg-[#EFE6DC] px-6 py-4 border-b border-[#D6C4B0] flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#7C5A3E]">Employee Job Designation Master</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              required
              placeholder="e.g. Manager, Operator, Labour..."
              value={name}
          ...
          <div>
            ...
            <textarea
              rows={3}
              placeholder="Roles, responsibilities, or job scope..."
              value={description}
          ...
          <div className="pt-4 border-t border-[#D6C4B0] flex items-center justify-end space-x-3">
            ...
            >
              {submitting ? 'Saving...' : position ? 'Update Position' : 'Create Position'}
            </button>
          </div>
        </form>
      </div>
