  ...
  if (usePostgres) {
    ...
    await runSqlite(`
      CREATE TABLE IF NOT EXISTS settlements (
          ...
          linked_invoices TEXT, -- JSON string array in SQLite
          created_at TEXT DEFAULT (datetime('now'))
      );
    `);
...
export async function dbQuery(text, params = []) {
  if (usePostgres) {
    ...
    // Adapt Postgres param format ($1, $2...) to SQLite (?, ?...)
    let paramIndex = 1;
    ...
    if (trimmed.startsWith('SELECT') || trimmed.startsWith('WITH')) {
      ...
      // parse JSON arrays if linked_invoices
      return rows.map(r => {
      ...
      // Modify linked_invoices params to JSON string if array
